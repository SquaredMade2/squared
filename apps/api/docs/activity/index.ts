const activityDocs = {
	"/api/activity/{taskId}": {
		get: {
			...
			description:
				"Find and return all activities associated with a specific task ID.",
			parameters: [
				{
					in: "path",
					name: "taskId",
					...
					required: true,
					description:
						"The ID of the task whose activities are to be retrieved",
				},
			],
			responses: {
				200: {
		...
		post: {
			...
			description:
				"Create a new activity (either a commit or task event) associated with a specific task ID.",
			parameters: [
				{
					in: "path",
					name: "taskId",
