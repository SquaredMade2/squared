...
const addWorkspace = async (
  ...
  try {
    ...
    return next(
      new AppError(
        "$$$ An error occurred while adding the workspace.",
        500
      )
    );
  }
...
const getWorkspace = async (
  ...
  if (!url && !id) {
    return next(
      new AppError("$$$ No workspace id or url provided $$$", 404)
    );
  }
...
const createTokenLink = async (
  ...
  try {
    ...
    if (!id) {
      return next(
        new AppError("User with that email does not exist", 422)
      );
    }
    if (!workspace) {
...
const joinWorkspaceThroughUniversalLink = async (
  ...
  try {
    ...
    if (userExists) {
      return next(
        new AppError("User already exists in workspace", 404)
      );
    }
    ...
    if (updatedWorkspace) {
      const updatedWorkspaceWithTransformedDates =
...
const verifyTokenToJoinWorkspace = async (
  ...
  try {
    ...
    if (!user) {
      return next(
        new AppError("No User is found with that email", 422)
      );
    }
    ...
    if (tokenIndex === -1) {
      return next(
        new AppError(
          "Link has been expired. Please try a new invitation",
          498
        )
      );
    }
...
const getAllWorkspaces = async (
  ...
  const workspaceData = await User.find({ _id: id }).populate(
    "workspaces"
  );
...
const workspaceExists = async (
  ...
  if (exists) {
    return next(
      new AppError("$$$ Workspace already exists. $$$", 404)
    );
  }
  res.sendStatus(200);
};
