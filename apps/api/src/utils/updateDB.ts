...
// import type IUsersRoles from "../interface/userRoles";
...
//         const newWorkspace: IUsersRoles[] = await Promise.all(workspace.users.map(async (user) => {
//           const userFound = await User.findOne({_id: user.user})
//           if (userFound) {
//             const userId: Types.ObjectId = userFound.id
//             const userRole: IUsersRoles = { username: userFound?.name, user: userId, role: user.role }
//             return userRole
//           }
//           const emptyUser: IUsersRoles = {username: '', user: user.user, role: ''}
//           return emptyUser
//         }))
