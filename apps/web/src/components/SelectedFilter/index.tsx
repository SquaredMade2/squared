...
export default function SelectedFilter({
  ...
  const [showChangeOptionDropDown, setShowChangeOptionDropDown] =
    useState(false);
  ...
  const deleteFilter = (
    taskAttributeTitle: keyof CurrentFilters,
    ...
    if (taskAttributeTitle === "effortEstimate") {
      updatedAttributes = (
        currentFilters[taskAttributeTitle] as number[]
      ).filter(
        (taskAttribute) =>
          taskAttribute !== Number.parseFloat(taskAttributeDeleted)
      );
    } else {
      updatedAttributes = (
        currentFilters[taskAttributeTitle] as (string | null)[]
      ).filter((taskAttribute) => taskAttribute !== taskAttributeDeleted);
    }
  ...
  return (
    <>
      <div className="flex items-center ml-3 text-foreground mt-1">
        <p className="text-sm text-foreground font-semibold mr-2">Filter for</p>
        <div className="py-1 px-2 text-sm text-foreground bg-card rounded-l-md border-t border-b border-l border-border">
          <div>{taskAttributeTitle}</div>
        </div>
        <div className="relative">
          <button
            type="button"
            className="py-1 px-2 text-sm text-foreground bg-card border-t border-b border-border hover:bg-accent hover:text-foreground cursor-pointer"
        ...
        <div className="relative">
          <button
            ...
            className="py-1 px-2 text-sm text-foreground font-semibold bg-card border-t border-b border-border hover:bg-accent cursor-pointer"
          >
            <div>
              {/* {taskAttribute.date ? (
								<p id="statusMenu">
        ...
        <button
          ...
          onClick={() => {
            ...
            deleteFilter(
              ...
              taskAttributeDeleted
            );
          }}
          className="py-1 px-2 mr-4 text-sm text-foreground bg-card rounded-r-md border hover:bg-accent hover:text-red-500 cursor-pointer"
        >
          clear
        </button>
      </div>
