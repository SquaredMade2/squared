		...
		return repositories
			...
			.map((repo) => {
				return (
					<button
						type="button"
						className="hover:bg-accent p-2 px-4 rounded-md"
						key={repo.id}
	...
	return (
		<div>
			<input
				...
				className="flex flex-row items-center bg-textField text-foreground w-80 h-12 px-2 rounded-lg"
			/>

			<div
				className={`absolute flex flex-col w-80 max-h-60 py-2 items-center rounded-b-lg ${
					toggleDropdown === true ? "" : "hidden"
				} bg-textField text-foreground overflow-auto`}
			>
				{repositories.length > 0 && displayedRepositories()}
			</div>
		</div>
	);
};
