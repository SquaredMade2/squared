...
const styles = {
	...
	textAddLabel:
		"ml-1.5 text-sm font-semibold text-card-foreground cursor-pointer",
	smallButton:
		"cursor-pointer border border-border hover mr-2 px-1.5 py-1 rounded-md shadow-md",
	...
	multipleLabelsText:
		"text-sm font-semibold text-card-foreground ml-2 cursor-pointer",
	multipleLabels: {
		1: "-mr-[5px]",
		2: "-mr-[5px]",
		3: "-mr-[5px]",
			...
			>
				...
				{newIssueLabels && newIssueLabels.length > 1 && (
					<div className={styles.labelContainer}>
						...
						<span
							className={styles.multipleLabelsText}
						>{`${newIssueLabels.length} labels`}</span>
					</div>
				)}
			</button>
		);
	...
	const issueSidebarButton = () => {
		return (
			<div>
				{sidebarLabels?.map((name: string) => {
					return (
						...
						>
							<LabelColor name={name} />
							<span
								className={`${styles.textSidebar} group-hover:text-foreground`}
							>
								{name}
							</span>
						</button>
				...
				<button
					type="button"
					className={styles.buttonSidebarAdd}
					onClick={handleButtonClick}
				>
