...
const WidgetLabelDropdown = ({
  label,
  setFilter,
  getColorFromLabel,
}: WidgetLabelDropdownProps) => {
  ...
  return (
    <div
      className={`flex flex-row h-13 items-center ${
        checked === true ? "bg-accent" : ""
      } hover:bg-accent duration-200 p-2 rounded-xl`}
      key={label[0]}
      onClick={changeChecked}
    >
      <div className="flex flex-row justify-center items-center rounded-full border border-border p-1 pr-3">
        <div
      ...
      <header className="ml-auto">{label[1]}</header>
    </div>
  );
};
