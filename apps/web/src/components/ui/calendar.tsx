...
const Chevron = ({ orientation = "left", ...props }) => {
	return orientation === "left" ? (
		<ChevronLeft className="relative z-10 size-4" {...props} />
	) : (
		<ChevronRight className="relative z-10 size-4" {...props} />
	);
};
	...
	return (
		<DayPicker
			...
			classNames={{
				...
				chevron: "group-hover:stroke-white",
				nav: "space-x-1 flex items-center",
				button_previous: cn(
					buttonVariants({ variant: "outline" }),
					"h-7 w-7 absolute top-3.5 left-[22px] z-20 bg-transparent p-0 opacity-50 hover:opacity-100 group",
				),
				button_next: cn(
					buttonVariants({ variant: "outline" }),
					"h-7 w-7 absolute top-3.5 right-[22px] z-20 bg-transparent p-0 opacity-50 hover:opacity-100 group",
				),
