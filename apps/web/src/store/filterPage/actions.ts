	...
	(dispatch: AppDispatch, getState: () => RootState) => {
		...
		switch (filter.group) {
			...
			case "labels":
				...
				break;
			case "dueDate": {
				const validComparison = filter.comparison as
					| "before"
					| "after"
					| undefined;
				updatedFilters = {
					...updatedFilters,
					dueDate: currentFilters.dueDate.some(
						(dateFilter) => dateFilter.name === filter.name,
					)
						? currentFilters.dueDate.filter(
								(dateFilter) => dateFilter.name !== filter.name,
							)
						: [
								...currentFilters.dueDate,
								{ name: filter.name, comparison: validComparison },
							],
				};
				break;
			}
			case "effortEstimate":
				updatedFilters = {
					...updatedFilters,
					effortEstimate: currentFilters.effortEstimate.includes(
						Number(filter.name),
					)
						? currentFilters.effortEstimate.filter(
								(item) => item !== Number(filter.name),
							)
