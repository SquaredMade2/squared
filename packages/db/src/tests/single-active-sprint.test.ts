...
describe("Single active sprint unique index test", () => {
    it("should not allow two active sprints for the same team", async () => {
        ...
        await db.delete(sprintsTable).where(
            inArray(sprintsTable.id, [sprint1Id, sprint2Id].filter(Boolean) as string[])
        );
        await db.delete(teamsTable).where(eq(teamsTable.id, teamId));
        await db.delete(workspacesTable).where(eq(workspacesTable.externalId, workspaceId));
    });
});