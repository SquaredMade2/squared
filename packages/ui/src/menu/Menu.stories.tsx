...
export const Styled = () => (
  <MenuWithAnchor>
    <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('undo')}>
      Undo
    </Menu.MenuItem>
    <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('redo')}>
      Redo
    </Menu.MenuItem>
    <Menu.MenuSeparator className={separatorClass()} />
    <Menu.MenuItem className={itemClass()} disabled onSelect={() => window.alert('cut')}>
      Cut
    </Menu.MenuItem>
    <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('copy')}>
      Copy
    </Menu.MenuItem>
    <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('paste')}>
      Paste
    </Menu.MenuItem>
  </MenuWithAnchor>
);
    ...
    <DirectionProvider dir={rtl ? 'rtl' : 'ltr'}>
      ...
      <MenuWithAnchor>
        ...
        <Submenu open={open1} onOpenChange={setOpen1} animated={animated}>
          ...
          <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('one')}>
            One
          </Menu.MenuItem>
          <Submenu open={open2} onOpenChange={setOpen2} animated={animated}>
            ...
            <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('two')}>
              Two
            </Menu.MenuItem>
            <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('three')}>
              Three
            </Menu.MenuItem>
            <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('four')}>
              Four
            </Menu.MenuItem>
            <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('five')}>
              Five
            </Menu.MenuItem>
            <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('six')}>
              Six
            </Menu.MenuItem>
          </Submenu>
  ...
  <MenuWithAnchor>
    {foodGroups.map((foodGroup, index) => (
      <Menu.MenuGroup key={index}>
        {foodGroup.label && (
          <Menu.MenuLabel className={labelClass()} key={foodGroup.label}>
            {foodGroup.label}
          </Menu.MenuLabel>
        )}
        ...
        {index < foodGroups.length - 1 && <Menu.MenuSeparator className={separatorClass()} />}
      </Menu.MenuGroup>
    ))}
  </MenuWithAnchor>
);
  ...
  <>
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 100 }}>
      ...
      <div>
        ...
        <MenuWithAnchor>
          {suits.map((suit) => (
            <Menu.MenuItem key={suit.emoji} className={itemClass()} textValue={suit.label}>
              <span role="img" aria-label={suit.label}>
                {suit.emoji}
              </span>
              {suit.label}
...
export const RadioItems = () => {
  ...
  return (
    <MenuWithAnchor>
      <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('minimize')}>
        Minimize window
      </Menu.MenuItem>
      <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('zoom')}>
        Zoom
      </Menu.MenuItem>
      <Menu.MenuItem className={itemClass()} onSelect={() => window.alert('smaller')}>
        Smaller
      </Menu.MenuItem>
      <Menu.MenuSeparator className={separatorClass()} />
      <Menu.MenuRadioGroup value={file} onValueChange={setFile}>
        {files.map((file) => (
          <Menu.MenuRadioItem key={file} className={itemClass()} value={file}>
            {file}
            <Menu.MenuItemIndicator>
              <TickIcon />
            </Menu.MenuItemIndicator>
    ...
    <>
      ...
      <MenuWithAnchor className={animatedContentClass()} open={open}>
        {checkboxItems.map(({ label, state: [checked, setChecked], disabled }) => (
          ...
          >
            {label}
            <Menu.MenuItemIndicator className={animatedItemIndicatorClass()}>
              <TickIcon />
            </Menu.MenuItemIndicator>
          </Menu.MenuCheckboxItem>
        ))}
...
type MenuProps = Omit<
  React.ComponentProps<typeof Menu.Menu> & React.ComponentProps<typeof Menu.MenuContent>,
...
> = (props) => {
  ...
  return (
    <Menu.MenuSub open={open} onOpenChange={onOpenChange}>
      <Menu.MenuSubTrigger className={subTriggerClass()} disabled={disabled}>
        {heading} →
      </Menu.MenuSubTrigger>
      <Menu.MenuPortal>
        <Menu.MenuSubContent
