...
>((props: ScopedProps<ScrollAreaProps>, forwardedRef) => {
  ...
  const [content, setContent] = React.useState<HTMLDivElement | null>(
    null
  );
...
interface ScrollAreaScrollbarHoverProps
...
type ScrollAreaScrollbarScrollElement =
...
type ScrollAreaScrollbarAutoElement =
  ScrollAreaScrollbarVisibleElement;
...
>(
  (
    props: ScopedProps<ScrollAreaScrollbarAutoProps>,
    forwardedRef
  ) => {
    const context = useScrollAreaContext(
      SCROLLBAR_NAME,
...
type ScrollAreaScrollbarVisibleElement =
...
>(
  (
    ...
    const thumbRef = React.useRef<ScrollAreaThumbElement | null>(
      null
    );
...
>(
  (
    props: ScopedProps<ScrollAreaScrollbarAxisProps>,
    forwardedRef
  ) => {
    ...
    return (
      <ScrollAreaScrollbarImpl
        ...
        style={{
          ...
          left:
            context.dir === "rtl"
              ? "var(--squared-scroll-area-corner-width)"
              : 0,
          right:
            context.dir === "ltr"
              ? "var(--squared-scroll-area-corner-width)"
        ...
        onWheelScroll={(event, maxScrollPos) => {
          if (context.viewport) {
            const scrollPos =
              context.viewport.scrollLeft + event.deltaX;
    ...
    return (
      <ScrollAreaScrollbarImpl
        ...
        onWheelScroll={(event, maxScrollPos) => {
          if (context.viewport) {
            const scrollPos =
              context.viewport.scrollTop + event.deltaY;
            props.onWheelScroll(scrollPos);
            // prevent window scroll when wheeling on scrollbar
            if (
              isScrollingWithinScrollbarBounds(
...
>(
  (
    props: ScopedProps<ScrollAreaScrollbarImplProps>,
    forwardedRef
  ) => {
    const {
      __scopeScrollArea,
...
// https://developer.mozilla.org/en-US/docs/Mozilla/Performance/Scroll-linked_effects
const addUnlinkedScrollListener = (
  node: HTMLElement,
  handler = () => {}
) => {
