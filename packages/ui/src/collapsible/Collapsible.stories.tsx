...
export const Controlled = () => {
  ...
  return (
    <Collapsible.Collapsible open={open} onOpenChange={setOpen} className={rootClass()}>
      <Collapsible.CollapsibleTrigger className={triggerClass()}>
        {open ? 'close' : 'open'}
      </Collapsible.CollapsibleTrigger>
...
const slideDown = keyframes({
  from: { height: 0 },
  to: { height: 'var(--sandy-collapsible-content-height)' },
});

const slideUp = keyframes({
  from: { height: 'var(--sandy-collapsible-content-height)' },
  to: { height: 0 },
});

const openRight = keyframes({
  from: { width: 0 },
  to: { width: 'var(--sandy-collapsible-content-width)' },
});

const closeRight = keyframes({
  from: { width: 'var(--sandy-collapsible-content-width)' },
  to: { width: 0 },
});
