...
export type FocusDayPickerContext = Partial<
  Pick<DayPickerContextValue, 'ISOWeek' | 'weekStartsOn' | 'fromDate' | 'toDate' | 'locale'>
>;
...
export function getNextFocus(focusedDay: Date, options: FocusDayOptions): Date {
  ...
  const moveFns = {
    ...
    startOfWeek: (date: Date) =>
      context.ISOWeek ? startOfISOWeek(date) : startOfWeek(date, { locale, weekStartsOn }),
    endOfWeek: (date: Date) =>
      context.ISOWeek ? endOfISOWeek(date) : endOfWeek(date, { locale, weekStartsOn }),
  };
