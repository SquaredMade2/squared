...
import { InternalModifier, InternalModifiers, Modifiers } from '../../../types/Modifiers';
...
describe.each(modifiersTest)(
  ...
  (modifierTest) => {
    ...
    const context = {
      fromDate: modifierTest.fromDate ? parseISO(modifierTest.fromDate) : undefined,
    ...
    test(`should skip the ${modifierTest.modifierName} day`, () => {
      ...
      expect(format(nextFocus, 'yyyy-MM-dd')).toBe(modifierTest.expectedNextFocus);
    });
  },
);
