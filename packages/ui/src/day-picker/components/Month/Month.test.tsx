...
describe("when rendered", () => {
  ...
  test("the caption id should be the aria-labelledby of the grid", () => {
    ...
    const gridLabelledBy =
      getMonthGrid().getAttribute("aria-labelledby");
...
describe("when using a custom Caption component", () => {
  ...
  test("it should render the custom component instead", () => {
    expect(
      screen.getByText("custom caption foo")
    ).toBeInTheDocument();
  });
});
...
describe("when dir is ltr", () => {
  ...
  describe.each(testLtr)(
    ...
    ({ monthProps, dayPickerProps, expected, notExpected }) => {
      ...
      test.each(expected)(
        "the root should have the %s class",
        (name) => expect(root).toHaveClass(testClassNames[name])
      );
      test.each(expected)(
        "the root should have the %s style",
        (name) => expect(root).toHaveStyle(testStyles[name])
      );
      test.each(notExpected)(
        "the root should not have the %s class",
        (name) => expect(root).not.toHaveClass(testClassNames[name])
      );
    }
  );
});
