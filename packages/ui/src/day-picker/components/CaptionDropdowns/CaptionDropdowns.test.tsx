...
describe('when rendered with custom styles or classnames', () => {
  ...
  beforeEach(() => {
    ...
    const view = customRender(<CaptionDropdowns displayMonth={today} />, dayPickerProps);
...
describe('when a month is selected', () => {
  ...
  describe('from the months drop-down', () => {
    ...
    beforeEach(async () => {
      await user.selectOptions(getMonthDropdown(), newMonth.getMonth().toString());
    });
  ...
  describe('from the years drop-down', () => {
    ...
    beforeEach(async () => {
      await user.selectOptions(getYearDropdown(), newYear.getFullYear().toString());
    });
    test('should call the `onMonthChange` callback', () => {
