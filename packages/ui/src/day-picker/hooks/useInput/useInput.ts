...
import { DayClickEventHandler, MonthChangeEventHandler } from '../../types/EventHandlers';
...
export type InputDayPickerProps = Pick<
  ...
  'fromDate' | 'toDate' | 'locale' | 'month' | 'onDayClick' | 'onMonthChange' | 'selected' | 'today'
>;

export interface UseInputOptions
  extends Pick<
    DayPickerBase,
    'locale' | 'fromDate' | 'toDate' | 'fromMonth' | 'toMonth' | 'fromYear' | 'toYear' | 'today'
  > {
...
export function useInput(options: UseInputOptions = {}): UseInputValue {
  ...
  const defaultInputValue = defaultSelected ? _format(defaultSelected, format, { locale }) : '';
