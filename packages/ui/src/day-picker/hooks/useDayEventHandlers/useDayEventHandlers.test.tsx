...
describe.each<"single" | "multiple" | "range">([
  "single",
  "multiple",
  "range",
])('when calling "onClick" in "%s" selection mode', (mode) => {
  const activeModifiers: ActiveModifiers = {};
  const dayPickerProps = {
    mode,
...
describe('when calling "onFocus"', () => {
  const date = today;
  const activeModifiers: ActiveModifiers = {};
  const mouseEvent = {} as React.FocusEvent<
    HTMLButtonElement,
    Element
  >;
      ...
      describe(`when text direction is "${dir.toUpperCase()}"`, () => {
        describe(`when the shiftKey is ${shiftKey ? "" : "not"} pressed`, () => {
          ...
          test(`should call ${expectedMethod}`, () => {
            expect(
              mockedContexts.focus[expectedMethod]
