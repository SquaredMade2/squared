...
function renderHook(date: Date, activeModifiers: ActiveModifiers, dayPickerProps?: DayPickerProps) {
...
describe.each(tests)('when calling "%s"', (eventName, dayEventName) => {
  ...
  test(`${dayEventName} should have been called`, () => {
    ...
    expect(dayPickerProps[dayEventName]).toHaveBeenCalledWith(date, activeModifiers, mouseEvent);
  });
});
...
describe('when calling "onKeyDown"', () => {
  ...
  describe.each(tests)('when key is %s', (key, dir, shiftKey, expectedMethod) => {
    describe(`when text direction is "${dir.toUpperCase()}"`, () => {
      describe(`when the shiftKey is ${shiftKey ? '' : 'not'} pressed`, () => {
        const keyboardEvent = {
          key,
          shiftKey,
