...
import { EventName } from '../useDayEventHandlers';
...
describe('when rendering the today’s date', () => {
  ...
  test('the div should include the default class name', () => {
    ...
    expect(result.current.divProps.className?.split(' ')).toContain(defaultClassNames.day);
  });
  test('the button should include the default class name', () => {
    const result = renderHook(date, displayMonth);
    expect(result.current.buttonProps.className?.split(' ')).toContain(defaultClassNames.day);
  });
  ...
  test.each(testEvents)('the button should have the "%s" event handler', (eventName) => {
    const result = renderHook(date, displayMonth);
...
describe('when "modifiersStyles" is passed in', () => {
  ...
  test('the div props should include the modifiers style', () => {
    const result = renderHook(date, date, dayPickerProps);
    expect(result.current.divProps.style).toStrictEqual(dayPickerProps.modifiersStyles.foo);
  });
  test('the button props should include the modifiers style', () => {
    const result = renderHook(date, date, dayPickerProps);
    expect(result.current.buttonProps.style).toStrictEqual(dayPickerProps.modifiersStyles.foo);
  });
});
describe('when "styles.day" is passed in', () => {
  ...
  test('the div props should include the style', () => {
    const result = renderHook(date, date, dayPickerProps);
    expect(result.current.divProps.style).toStrictEqual(dayPickerProps.styles.day);
  });
  test('the button props should include the style', () => {
    const result = renderHook(date, date, dayPickerProps);
    expect(result.current.buttonProps.style).toStrictEqual(dayPickerProps.styles.day);
  });
});
...
describe('when "modifiersClassNames" is passed in', () => {
  ...
  test('the div props should include the modifiers classNames', () => {
    expect(result.current.divProps.className).toContain(dayPickerProps.modifiersClassNames.foo);
  });
  test('the button props should include the modifiers classNames', () => {
    expect(result.current.buttonProps.className).toContain(dayPickerProps.modifiersClassNames.foo);
  });
});
...
describe('when "classNames.day" is passed in', () => {
  ...
  test('the div props should include the class name', () => {
    expect(result.current.divProps.className).toContain(dayPickerProps.classNames.day);
  });
  test('the button props should include the class name', () => {
    expect(result.current.buttonProps.className).toContain(dayPickerProps.classNames.day);
  });
});
...
describe('when the day is not target of focus', () => {
  ...
  const result = renderHook(tomorrow, tomorrow, {}, { ...mockedContexts, focus: focusContext });
...
describe('when the day is target of focus', () => {
  ...
  const result = renderHook(date, date, {}, { ...mockedContexts, focus: focusContext });
