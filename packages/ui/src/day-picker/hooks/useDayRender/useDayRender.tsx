...
export function useDayRender(
  ...
  const isButton = Boolean(dayPicker.onDayClick || dayPicker.mode !== 'default');
  ...
  const isHidden = Boolean(
    (activeModifiers.outside && !dayPicker.showOutsideDays) || activeModifiers.hidden,
  );

  const DayContentComponent = dayPicker.components?.DayContent ?? DayContent;
  const children = (
    <DayContentComponent date={day} displayMonth={displayMonth} activeModifiers={activeModifiers} />
  );
  ...
  const isFocused = focusContext.focusedDay && isSameDay(focusContext.focusedDay, day);
