...
import * as Slider from "@squared-ui/react-slider";
		...
		<>
			<label>
				...
				<input
					...
					onChange={(event) =>
						setMinStepsBetweenThumbs(Number(event.target.value))
					}
					style={{ width: 30 }}
				/>
			</label>
	...
	return (
		...
		>
			...
			<fieldset>
				...
				<Slider.Root
					defaultValue={[data.price.min, data.price.max]}
					className={rootClass()}
				>
					<Slider.Track className={trackClass()}>
						<Slider.Range className={rangeClass()} />
