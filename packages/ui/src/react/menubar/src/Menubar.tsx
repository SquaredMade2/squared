...
import * as MenuPrimitive from "@squared-ui/react-menu";
import { createMenuScope } from "@squared-ui/react-menu";
import * as RovingFocusGroup from "@squared-ui/react-roving-focus";
import { createRovingFocusGroupScope } from "@squared-ui/react-roving-focus";
...
const [createMenubarContext, createMenubarScope] = createContextScope(
	MENUBAR_NAME,
...
const MenubarPortal: React.FC<MenubarPortalProps> = (
	props: ScopedProps<MenubarPortalProps>,
) => {
	...
	return (
		<MenuPrimitive.Content
				...
				(event) => {
					if (["ArrowRight", "ArrowLeft"].includes(event.key)) {
						...
						const targetIsSubTrigger = target.hasAttribute(
							"data-squared-menubar-subtrigger",
						);
						const isKeyDownInsideSubMenu =
							target.closest("[data-squared-menubar-content]") !==
							event.currentTarget;
			...
			style={{
				...
				...{
					"--squared-menubar-content-transform-origin":
						"var(--squared-popper-transform-origin)",
					"--squared-menubar-content-available-width":
						"var(--squared-popper-available-width)",
					"--squared-menubar-content-available-height":
						"var(--squared-popper-available-height)",
					"--squared-menubar-trigger-width":
						"var(--squared-popper-anchor-width)",
					"--squared-menubar-trigger-height":
...
const MenubarSub: React.FC<MenubarSubProps> = (
	props: ScopedProps<MenubarSubProps>,
) => {
	const {
		__scopeMenubar,
		children,
