...
import * as Select from "@squared-ui/react-select";
import { Label } from "@squared-ui/react-label";
				...
				<Select.Root defaultValue="item-25">
					...
					<Select.Portal>
						<Select.Content
							...
							sideOffset={5}
						>
							<Select.ScrollUpButton className={scrollUpButtonClass()}>
								▲
											...
											<Select.Group className={groupStyles()}>
												...
												{foodGroup.foods.map((food) => (
													<Select.Item
														key={food.value}
														className={
															hasLabel ? itemInGroupClass() : itemClass()
														}
														value={food.value}
													>
														<Select.ItemText>{food.label}</Select.ItemText>
		...
		<div style={{ padding: 50 }}>
			...
			<Select.Root defaultValue="18-40">
				<Select.Trigger
					...
					aria-labelledby="age-aria-labelledby"
				>
					<Select.Value />
					<Select.Icon />
				</Select.Trigger>
...
ChromaticShortOptionsPaddedContent.parameters = {
...
ChromaticShortOptionsPaddedViewport.parameters = {
	chromatic: { disable: false },
};
...
ChromaticLongOptionsPaddedContent.parameters = {
	chromatic: { disable: false },
};
...
ChromaticLongOptionsPaddedViewport.parameters = {
	chromatic: { disable: false },
};
...
ChromaticBottomLastPaddedViewport.parameters = {
	chromatic: { disable: false },
};
		...
		<Select.Root defaultValue={String(selected)} open>
			...
			<Select.Portal>
				...
				>
					...
					<Select.Viewport
						className={
							paddedElement === "viewport" ? viewportClass() : undefined
						}
					>
					...
					<Select.ScrollDownButton
						...
						style={
							paddedElement === "content" ? { marginBottom: -5 } : undefined
						}
					>
						▼
					</Select.ScrollDownButton>
...
const ChromaticStoryShortOptions = ({
	paddedElement,
}: { paddedElement: PaddedElement }) => (
	<div
		style={{
...
const ChromaticStoryLongOptions = ({
	paddedElement,
}: { paddedElement: PaddedElement }) => (
	<div
		style={{
...
const ChromaticStoryBottomLast = ({
	paddedElement,
}: { paddedElement: PaddedElement }) => (
...
const contentClass = css({
	...
	minWidth: "var(--squared-select-trigger-width)",
	maxHeight: "var(--squared-select-content-available-height)",
});
