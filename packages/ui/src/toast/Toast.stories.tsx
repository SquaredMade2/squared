...
export const Controlled = () => {
  ...
  return (
    <Toast.Provider>
      ...
      <button onClick={() => setSavedCount((count) => count + 1)}>
        save
      </button>
      <ToastUpgradeAvailable
        open={hasUpgrade}
        onOpenChange={setHasUpgrade}
      />
...
Chromatic.parameters = {
  chromatic: { disable: false, delay: SNAPSHOT_DELAY },
};
...
const ToastUpgradeAvailable = (
  ...
  <Toast.Root className={rootClass()} {...props}>
    ...
    <Toast.Description className={descriptionClass()}>
      We've just released squared version 3.0
    </Toast.Description>
    <Toast.Action
...
const slideRight = keyframes({
  from: { transform: "translateX(var(--squared-toast-swipe-end-x))" },
...
const slideUp = keyframes({
  from: { transform: "translateY(var(--squared-toast-swipe-end-y))" },
  to: {
...
const animatedRootClass = css(rootClass, {
  ...
  '&[data-swipe="move"]': {
    transform:
      "translate(var(--squared-toast-swipe-move-x), var(--squared-toast-swipe-move-y))",
  },
  '&[data-swipe="cancel"]': {
