...
const [createToastContext, createToastScope] = createContextScope(
  "Toast",
...
const ToastProvider: React.FC<ToastProviderProps> = (
  ...
  const [viewport, setViewport] =
    React.useState<ToastViewportElement | null>(null);
    ...
    return (
      <Presence present={forceMount || open}>
        <ToastImpl
          ...
          onSwipeMove={composeEventHandlers(
            props.onSwipeMove,
            (event) => {
          ...
          onSwipeEnd={composeEventHandlers(
            props.onSwipeEnd,
            (event) => {
              const { x, y } = event.detail.delta;
...
type PrimitiveListItemProps = React.ComponentPropsWithoutRef<
  typeof Primitive.li
>;
interface ToastImplProps
  extends ToastImplPrivateProps,
...
const ToastImpl = React.forwardRef<ToastImplElement, ToastImplProps>(
  (props: ScopedProps<ToastImplProps>, forwardedRef) => {
    ...
    const [node, setNode] = React.useState<ToastImplElement | null>(
      null
    );
...
function getAnnounceTextContent(container: HTMLElement) {
  ...
  for (const node of childNodes) {
    ...
    if (isHTMLElement(node)) {
      ...
      const isExcluded =
        node.dataset.sandyToastAnnounceExclude === "";
...
>(
  ...
  detail: { originalEvent: ReactEvent } & (E extends CustomEvent<
    infer D
  >
    ? D
    : never),
  ...
  const currentTarget = detail.originalEvent
