...
import * as ToggleGroup from "@squared-ui/react-toggle-group";
...
describe("given a single ToggleGroup", () => {
	...
	beforeEach(() => {
		...
		rendered = render(
			<ToggleGroupTest type="single" onValueChange={handleValueChange} />,
		);
...
describe("given a multiple ToggleGroup", () => {
	...
	beforeEach(() => {
		handleValueChange = jest.fn();
		rendered = render(
			<ToggleGroupTest type="multiple" onValueChange={handleValueChange} />,
		);
		one = rendered.getByText("One");
		two = rendered.getByText("Two");
	});
...
const ToggleGroupTest = (
	props: React.ComponentProps<typeof ToggleGroup.Root>,
) => (
