...
// biome-ignore lint/suspicious/noExplicitAny: Value is not known at compile time
...
export const getDeep = (object: object, path: (string | number)[]): object => {
	...
	for (let i = 0; i < path.length; i++) {
		...
		if (isSet(newObject)) {
			...
			newObject = newObject[key as keyof typeof newObject];
		}
	}
...
export const setDeep = (
	object: object,
	path: (string | number)[],
	// biome-ignore lint/suspicious/noExplicitAny: Value could be any type
	mapper: (v: any) => any,
	// biome-ignore lint/suspicious/noExplicitAny: Value could be any type
): any => {
	validatePath(path);
