...
// {{title .Name}}Service represents the RPC client for the {{.Name}} service
type {{title .Name}}Service struct {
	baseURL string
	client  *http.Client
}

// New{{title .Name}}Service creates a new instance of {{title .Name}}Service
func New{{title .Name}}Service(baseURL string) *{{title .Name}}Service {
...
{{range .Interfaces}}
{{if ne .InputType "undefined"}}
// {{title .MethodName}}Request represents the request for {{.MethodName}} method
type {{title .MethodName}}Request {{zodToGo .RequestSchema}}
{{end}}

{{if ne .OutputType "void"}}
// {{title .MethodName}}Response represents the response from {{.MethodName}} method
type {{title .MethodName}}Response {{zodToGo .ResponseSchema}}
{{end}}

// {{.MethodName}} calls the {{.MethodName}} RPC method
func (s *{{title $.Name}}Service) {{.MethodName}}(ctx context.Context{{if ne .InputType "undefined"}}, req {{title .MethodName}}Request{{end}}) ({{if eq .OutputType "void"}}error{{else}}{{title .MethodName}}Response, error{{end}}) {
	endpoint := fmt.Sprintf("%s/{{$.Name}}/{{.MethodName}}", s.baseURL)

	{{if ne .InputType "undefined"}}
	reqBody, err := json.Marshal(req)
	if err != nil {
		return {{if ne .OutputType "void"}}{{title .MethodName}}Response{}, {{end}}fmt.Errorf("error marshaling request: %w", err)
	}
	{{end}}

	httpReq, err := http.NewRequestWithContext(ctx, "POST", endpoint, {{if ne .InputType "undefined"}}bytes.NewBuffer(reqBody){{else}}nil{{end}})
	if err != nil {
		return {{if ne .OutputType "void"}}{{title .MethodName}}Response{}, {{end}}fmt.Errorf("error creating request: %w", err)
	}

	requestID := make([]byte, 6)
	if _, err := rand.Read(requestID); err != nil {
		return {{if ne .OutputType "void"}}{{title .MethodName}}Response{}, {{end}}fmt.Errorf("error generating request ID: %w", err)
	}
	...
	if err != nil {
		return {{if ne .OutputType "void"}}{{title .MethodName}}Response{}, {{end}}fmt.Errorf("error sending request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return {{if ne .OutputType "void"}}{{title .MethodName}}Response{}, {{end}}fmt.Errorf("unexpected status code: %d, body: %s", resp.StatusCode, string(body))
	}

	{{if eq .OutputType "void"}}
	return nil
	{{else}}
	var response {{title .MethodName}}Response
	if err := json.NewDecoder(resp.Body).Decode(&response); err != nil {
		return {{title .MethodName}}Response{}, fmt.Errorf("error decoding response: %w", err)
	}
